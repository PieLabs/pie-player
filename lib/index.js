...
class PiePlayer extends HTMLElement {
   ...
   * @property {Summary} summary The summary
   * @property {PieScore[]} pies The pie scores
   * @property {Weight[]} weights The weights
   */

  /**
   * @typedef {Object} Summary 
   * @property {number} min - the min - always 0?
   * @property {number} max - the max number of points
   * @property {number} score - the score in points
   * @property {number} percentage - the percentage value aka ((score/max) * 100)
   */

  /**
   * @typedef {Object} PieScore
   * @property {string} id The id
   * @property {number} score The score between 0 and 1.0  
   */
   ...
   * Get the outcomes.
   * @returns {Promise}
   * @fulfil {Outcome} - The outcome object 
   */
  outcomes() {
  ...
  _registerPieInstance(id, el) {
    ...
    if (!el) {
      throw new Error(`Can't find element for pie-id: ${id}`);
    }
