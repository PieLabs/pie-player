...
export default class PiePlayer extends HTMLElement {
  ...
  constructor() {
    ...
    this._dispatchStatusChanged = debounce(statusChange.bind(this), 50, { leading: false, trailing: true });
    ...
    this.addEventListener('pie.register', (event) => {
      ...
      this._pies[id].addEventListener('response-changed', e => {
      ...
      this._pies[id].addEventListener('model-updated', e => {
        this._statuses[id] = this._statuses[id] || { complete: undefined }
        const status = this._statuses[id];
        status.complete = e.detail.complete;
        this._dispatchStatusChanged();
